...
import 'package:vss_card_reader/src/card_reader_nfc_manager/the_viet_card_reader_nfc_manager.dart';
import 'package:vss_card_reader/src/card_reader_nfc_manager/other_card_reader_nfc_manager.dart';
...
typedef NfcResultDataCallback = Future<void> Function(String result);
...
class NfcManager {
  ...
  NfcResultDataCallback? _onResultDataDiscovered;
  ...
  Future<void> startSession({
    ...
    NfcResultDataCallback? onNfcResultDataCallback,
    Set<NfcPollingOption>? pollingOptions,
    String? alertMessage,
    NfcErrorCallback? onError,
  }) async {
    _onDiscovered = onDiscovered;
    _onResultDataDiscovered = onNfcResultDataCallback;
    _onError = onError;
  ...
  void _handleOnDiscovered(MethodCall call) async {
    ...
    if (isoDep != null) {
      var selectData = await isoDep.transceive(
          data: Uint8List.fromList(Constans.apduTheViet));
      if (selectData.length > 20) {
        TheVietCardReaderNfcManager theVietCardReaderNfcManager =
            TheVietCardReaderNfcManager(tag);
        dataResult = await theVietCardReaderNfcManager.readData();
      } else {
        String hexSelectData = HexUtil.bytes2Hex(selectData);
        if (hexSelectData.toUpperCase() == "6A82") {
          OtherCardReaderNfcManager otherCardReaderNfcManager =
              OtherCardReaderNfcManager(tag);
          dataResult = await otherCardReaderNfcManager.readData();
        }
      }
    } else {
      TheVietCardReaderNfcManager theVietCardReaderNfcManager =
    ...
    await _onResultDataDiscovered?.call(dataResult);
